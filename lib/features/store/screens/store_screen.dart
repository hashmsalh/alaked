...
class _StoreScreenState extends State<StoreScreen> {
  ...
  void initState() {
    ...
    // 🔥 مهم جدًا: تصفير المتجر السابق
    Get.find<StoreController>().resetCurrentStore();
  ...
  Future<void> initDataCall() async {
    ...
    storeController.resetFilter(isUpdate: false);
    if (storeController.isSearching) {
      storeController.changeSearchStatus(isUpdate: false);
    }
    storeController.hideAnimation();

    // 🔴 حالة Deep Link بدون ID (slug فقط)
    if (widget.store == null && widget.slug.isNotEmpty) {
      await storeController.getStoreDetailsBySlug(widget.slug);
    }

    // 🟢 الحالة الطبيعية (ID موجود)
    else if ((storeController.store?.id ?? widget.store?.id ?? 0) > 0) {

      await storeController.getStoreDetails(
        Store(id: storeController.store?.id ?? widget.store!.id),
        widget.fromModule,
        slug: widget.slug,
      );
    ...
    if (splash.module != null && splash.module!.id != null) {
    ...
    if (storeId > 0) {
      ...
      storeController.getStoreItemList(storeId, 1, 'all', false);
    }
                                  ...
                                  child: Row(children: [
                                    ClipRRect(
                                      ...
                                      child: Stack(children: [
                                        ...
                                        storeController.isStoreOpenNow(store.active ?? false, store.schedules)
                                        ...
                                        Get.toNamed(
                                          RouteHelper.getChatRoute(
                                            notificationBody: NotificationBodyModel(
                                              restaurantId: store!.vendorId, // ✅ الصحيح
                                            ),
                                            user: User(
                                              id: store!.vendorId,
                                              fName: store!.name,
                                              lName: '',
                                              imageFullUrl: store!.logoFullUrl,
                                            ),
                                            conversationID: null,
                          ...
                          SizedBox(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Container(
                                  height: (storeController.categoryList?.length ?? 0) * 50,
                                  width: 1,
                                            ...
                                            onSubmitted: (String? value) {
                                              if(value!.isNotEmpty) {
                                                Get.find<StoreController>().getStoreSearchItemList(
                                                  _searchController.text.trim(),(storeController.store?.id ?? widget.store?.id)
                                                    .toString(), 1, storeController.type,
                                                );
                                              }
                                            } ,
                                        ...
                                        return Padding(
                                          ...
                                          child:  CustomCheckBoxWidget(
                                            ...
                                            onClick: () {
                                              if(storeController.isSearching){
                                                storeController.getStoreSearchItemList(
                                                  storeController.searchText,(storeController.store?.id ?? widget.store?.id)
.toString(), 1, Get.find<ItemController>().itemTypeList[index],
                                                );
                                              } else {
                              ...
                              PaginatedListView(
                                ...
                                onPaginate: (int? offset) async {
                                  ...
                                  if (storeId <= 0 || offset == null) return;
                                ...
                                itemView: WebItemsView(
                                  ...
                                  items: storeController.isSearching
                                      ...
                                      : (storeController.categoryList?.isNotEmpty ?? false)
              ...
              SliverToBoxAdapter(child: Container(
                ...
                child: PaginatedListView(
                  ...
                  itemView: ItemsView(
                    ...
                    items: (storeController.categoryList?.isNotEmpty ?? false) &&
        ...
        floatingActionButton: GetBuilder<StoreController>(
            builder: (storeController) {
              ...
              final bool canShowFab =
                  splash.configModel != null &&
                      splash.configModel!.moduleConfig != null &&
                      splash.configModel!.moduleConfig!.module != null &&
                      splash.configModel!.moduleConfig!.module!.orderAttachment == true &&
                      splash.configModel!.prescriptionStatus == true &&
                      storeController.store != null &&
                      storeController.store!.prescriptionOrder == true &&
                      AuthHelper.isLoggedIn();

              return Visibility(
                visible: storeController.showFavButton && canShowFab,
                child: Container(
                  decoration: BoxDecoration(
                    color: Theme.of(context).cardColor,
                    borderRadius: BorderRadius.circular(Dimensions.radiusSmall),
