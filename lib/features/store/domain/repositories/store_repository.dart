...
class StoreRepository implements StoreRepositoryInterface {
    ...
    switch(source) {
      ...
      case DataSourceEnum.local:
        ...
        if (cacheResponseData != null) {
          ...
          // 🔁 لو ما فيه كاش → نرجع مباشرة للـ API
  ...
  Future<ItemModel?> getStoreItemList({
    ...
    final String cacheId =
        '${AppConstants.storeItemUri}'
        '?store_id=$storeID'
        '&category_id=$categoryID'
        '&type=$type'
        '&module=${Get.find<SplashController>().module?.id ?? Get.find<SplashController>().cacheModule?.id}'
        '&offset=$offset';
      ...
      case DataSourceEnum.client:
        Response response = await apiClient.getData(
          '${AppConstants.storeItemUri}'
              ...
              '&filter=$filterString'
              '&rating_count=${rating ?? ''}'
              '&min_price=${lowerValue ?? ''}'
              '&max_price=${upperValue ?? ''}',
        );
