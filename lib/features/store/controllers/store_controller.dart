...
class StoreController extends GetxController implements GetxService {
  ...
  /// 🔄 إعادة تعيين بيانات المتجر الحالي (مهم عند فتح متجر جديد)
  ...
  double getRestaurantDistance(osm.LatLng storeLatLng){
    double distance = 0;
  ...
  Future<Store?> getStoreDetails(Store store, bool fromModule, {bool fromCart = false, String slug = ''}) async {
    ...
    if(store.name != null) {
      ...
      if (storeDetails != null) {
        ...
        if(slug.isNotEmpty){
          await Get.find<LocationController>().setStoreAddressToUserAddress(osm.LatLng(double.parse(_store!.latitude!), double.parse(_store!.longitude!)));
        }
        if(fromModule) {
  ...
  /// ✅ جلب بيانات المتجر عن طريق slug (Deep Link)
  Future<Store?> getStoreDetailsBySlug(String slug) async {
    _categoryIndex = 0;
    _isLoading = true;
    _store = null;
    update();
    ...
    if (storeDetails != null) {
      ...
      // تحديث موقع المستخدم على موقع المتجر (مهم للديب لينك)
      await Get.find<LocationController>()
          .setStoreAddressToUserAddress(
        osm.LatLng(
          double.parse(_store!.latitude!),
          double.parse(_store!.longitude!),
        ),
  ...
  void shareStore() {
    ...
    double distanceKm = 0;
    if (address != null && store?.latitude != null && store?.longitude != null) {
    ...
    final String shareUrl =
        '${AppConstants.webHostedUrl}/store/${store!.slug}'
        '?module_id=${module?.id}'
        '&module_type=${module?.moduleType}'
        '&store_zone_id=${address?.zoneId}';
