...
class HomeScreen extends StatefulWidget {
  ...
  static Future<void> loadData(bool reload, {bool fromModule = false}) async {
    ...
    /// =========================
    /// (B) تحميل متأخر – ما يعلّق الواجهة
    /// =========================
    Future.delayed(const Duration(milliseconds: 300), () {

      if (Get.find<SplashController>().module != null) {

        // منتجات وصور كثيرة (نخليها لاحق)
        Get.find<ItemController>().getPopularItemList(offset: '1');
        Get.find<ItemController>().getDiscountedItemList(offset: '1');
        Get.find<ItemController>().getReviewedItemList(offset: '1');
        Get.find<ItemController>().getRecommendedItemList(false, 'all', false);
        ...
        if (moduleType == AppConstants.pharmacy) {
          Get.find<ItemController>().getBasicMedicine(reload, false);
          Get.find<ItemController>().getCommonConditions(false).then((_) {
            final conditions = Get.find<ItemController>().commonConditions;
            if (conditions != null && conditions.isNotEmpty) {
              Get.find<ItemController>()
                  .getConditionsWiseItem(conditions.first.id!, false);
            }
          });
        }
      }
...
// الصفحة بدون Module
      if (splash.module == null &&
          splash.configModel?.module == null) {
...
// Parcel
      if (splash.module != null &&
          splash.configModel?.moduleConfig?.module?.isParcel == true) {
...
class _HomeScreenState extends State<HomeScreen> {
                            ...
                            child: GetBuilder<LocationController>(
                                builder: (locationController) {
                                  ...
                                  return Column(
                                      ...
                                      /// نوع العنوان
                                      Text(
                                      AuthHelper.isLoggedIn() && address != null
                                      ? (address.addressType ?? 'your_location').tr
                                      : 'your_location'.tr,
          ...
          floatingActionButton: GetBuilder<HomeController>(
            builder: (homeController) {
              if (!AuthHelper.isLoggedIn() ||
                  ...
                  homeController.cashBackOfferList!.isEmpty ||
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    final bool isPinnedNow = shrinkOffset >= maxExtent;

    // لا ننفّذ callback إلا إذا تغيّر الوضع فعليًا
    if (callback != null && isPinnedNow != _lastPinned) {
      _lastPinned = isPinnedNow;
      callback!(isPinnedNow);
    }
  ...
  // نمنع إعادة بناء الهيدر أثناء السكرول
  @override
  bool shouldRebuild(covariant SliverDelegate oldDelegate) => false;
}
