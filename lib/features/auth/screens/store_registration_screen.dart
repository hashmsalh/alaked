...
//  final TextEditingController _tinNumberController = TextEditingController();
  ...
  void initState() {
    super.initState();
    int defaultLangIndex = 0;
    for (int i = 0; i < _languageList!.length; i++) {
      if (_languageList![i].key == 'ar') {
        defaultLangIndex = i;
        break;
      }
    }
      ...
      child: Scaffold(
        ...
        body: SafeArea(child: GetBuilder<StoreRegistrationController>(builder: (storeRegController) {
          ...
          if (storeRegController.storeAddress != null) {
            // خزّن العنوان في العربية فقط (الافتراضية)
            int arIndex = 0;
            for (int i = 0; i < _languageList!.length; i++) {
              if (_languageList![i].key == 'ar') {
                arIndex = i;
            ...
            if (_addressController[arIndex].text.isEmpty) {
              _addressController[arIndex].text =
              storeRegController.storeAddress!;
            }
          }
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              ...
                              SizedBox(
                                ...
                                child: Align(
                                  ...
                                  child: TabBar(
                                    ...
                                    onTap: (int? value) {
                                      setState(() {
                                        // انسخ العنوان من العربية إذا كانت الخانة فاضية
                                        int arIndex = 0;
                                        for (int i = 0; i < _languageList!.length; i++) {
                                          if (_languageList![i].key == 'ar') {
                                            arIndex = i;
                          ...
                          Builder(
                            builder: (context) {
                              ...
                              return storeRegController.zoneList != null
                                  ? SelectLocationViewWidget(
                                key: _locationInfoScrollKey,
                                fromView: true,
                                addressController: _addressController[arIndex],
                                addressFocus: _addressFocus[arIndex],
                              )
                                  : Container(
      ...
      return Container(
        ...
        child: CustomButton(
          ...
          onPressed: (storeRegController.storeStatus == 0.1 && !ResponsiveHelper.isDesktop(context) && !storeRegController.inZone)
            ...
            bool hasAnyLanguageData = false;

            for (int i = 0; i < _languageList!.length; i++) {
              if (_nameController[i].text.trim().isNotEmpty &&
                  _addressController[i].text.trim().isNotEmpty) {
                hasAnyLanguageData = true;
                break;
              }
            }
            ...
            if(storeRegController.storeStatus == 0.1 || storeRegController.storeStatus == 0.6) {
              if(storeRegController.storeStatus == 0.1 && !ResponsiveHelper.isDesktop(context)){
                ...
                if(ResponsiveHelper.isDesktop(context)){
                  ...
                  if(storeRegController.restaurantLocation == null) {
                    showCustomSnackBar('set_vendor_location'.tr);
                  }else if(storeRegController.selectedZoneIndex == -1) {
                    showCustomSnackBar('please_select_zone'.tr);
                  }else if(storeRegController.selectedModuleIndex == -1) {
              ...
              int defaultLangDataIndex = 0;

              for (int i = 0; i < _languageList!.length; i++) {
                if (_nameController[i].text.trim().isNotEmpty &&
                    _addressController[i].text.trim().isNotEmpty) {
                  defaultLangDataIndex = i;
                  break;
                }
              }
                ...
                translation.add(
                  Translation(
                    ...
                    value: _nameController[index].text.trim().isNotEmpty
                        ...
                        : _nameController[defaultLangDataIndex].text.trim(),
                  ),
                );
                ...
                translation.add(
                  Translation(
                    ...
                    value: _addressController[index].text.trim().isNotEmpty
                        ...
                        : _addressController[defaultLangDataIndex].text.trim(),
                  ),
                );
              }
