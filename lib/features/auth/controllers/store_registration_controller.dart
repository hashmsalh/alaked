...
class StoreRegistrationController extends GetxController implements GetxService {
  ...
  osm.LatLng? _restaurantLocation;
  osm.LatLng? get restaurantLocation => _restaurantLocation;
 ...
 /* final List<FilePickerResult> _tinFiles = [];
  ...
  void validPassCheck(String pass, {bool isUpdate = true}) {
    _lengthCheck = pass.length >= 8;

    // نلغي باقي الشروط نهائياً
    _numberCheck = true;
  ...
  Future<void> getZoneList() async {
    ...
    _restaurantLocation = null; // مهم جداً
    _zoneIds = null;
  ...
  void setLocation( osm.LatLng location, {bool forStoreRegistration = false, int? zoneId}) async {
    ...
    _storeAddress = locationController.pickAddress;
  ...
  Future<void> registerStore(StoreBodyModel storeBody) async {
    ...
    try {
      ...
      Response? response = await storeRegistrationServiceInterface
          .registerStore(storeBody, _pickedLogo, _pickedCover, []);
      ...
      if (response != null && response.statusCode == 200) {
        ...
        showCustomSnackBar(
            response?.statusText ?? "فشل التسجيل، تحقق من البيانات"
        );
      }

    } catch (e) {

      showCustomSnackBar("حدث خطأ أثناء التسجيل");
