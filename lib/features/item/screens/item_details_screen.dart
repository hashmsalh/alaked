  ...
  Widget build(BuildContext context) {
    return GetBuilder<CartController>(builder: (cartController) {
      return GetBuilder<ItemController>(builder: (itemController) {
        ...
        bool hasValidCartIndex =
            itemController.cartIndex != -1 &&
                itemController.cartIndex < cartController.cartList.length;
        ...
        item != null &&
            ...
            itemController.variationIndex!.length == item.choiceOptions!.length
        ) {
          ...
          for (int index = 0; index < item.choiceOptions!.length; index++) {
            ...
            item.choiceOptions![index].options != null &&
                item.choiceOptions![index].options!.isNotEmpty &&
                itemController.variationIndex![index] < item.choiceOptions![index].options!.length
            ) {
              variationList.add(
          ...
          double priceWithDiscount =
              PriceConverter.convertWithDiscount(price ?? 0, discount, discountType) ?? 0;
          ...
          for (int index = 0; index < item.addOns!.length; index++) {
            if (
            index < itemController.addOnActiveList.length &&
                index < itemController.addOnQtyList.length &&
...
// ===== PRODUCT WITHOUT VARIATIONS =====
        if (item != null &&
            (item.choiceOptions == null || item.choiceOptions!.isEmpty)) {
          ...
          double priceWithDiscount =
              PriceConverter.convertWithDiscount(price, discount, discountType) ?? price;

          priceWithAddons = priceWithDiscount * itemController.quantity!;
        ...
        final bool isStoreModule = item != null && item.moduleType != 'food';

        final bool isStockEnabled =
            isStoreModule &&
