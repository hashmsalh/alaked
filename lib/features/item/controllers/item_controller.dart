...
class ItemController extends GetxController implements GetxService {
  ...
  void selectCommonCondition(int index) {
    if (_commonConditions == null || _commonConditions!.isEmpty) return;
  ...
  void _preparePopularItems(ItemModel? itemModel, String offset, bool firstTimeCategoryLoad) {
    ...
    _pageSize = itemModel.totalSize ?? 0;
    _isLoading = false;
    update();
  }
  ...
  void itemDirectlyAddToCart(
    ...
    getItemDetails(itemId: item!.id!).then((value) {
      ...
      // ===== 1️⃣ تحديد هل المنتج له أنواع أم لا =====
      bool hasVariation =
...
// 🔒 تجاهل المخزون تمامًا لقسم المطاعم
      if (_item?.moduleType != 'food') {
        if (!hasVariation) {
          isOutOfStock = (_item?.stock ?? 0) <= 0;
        }
      }
      ...
      // ===== 3️⃣ في حال لا توجد أنواع → إضافة مباشرة للسلة =====
      if (!hasVariation) {

        double price = _item!.price!;
        double discount = _item!.discount!;
        double discountPrice =
        PriceConverter.convertWithDiscount(price, discount, _item!.discountType)!;
      ...
      // ===== 4️⃣ في حال يوجد أنواع → افتح صفحة / BottomSheet =====
