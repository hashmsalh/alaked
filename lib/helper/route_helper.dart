...
class RouteHelper {
  ...
  static List<GetPage> routes = [
    ...
    GetPage(name: verification, page: () {
      ...
      print('=========cccccc=====route: ${Get.parameters['back_from_this']}');
    ...
    GetPage(name: store, page: () {
      ...
      final int? storeId = int.tryParse(idParam ?? '');
      final uri = Uri.parse(Get.currentRoute);
      String slug = '';

// 🔹 slug من المسار /store/slug
      if (uri.pathSegments.isNotEmpty && uri.pathSegments[0] == 'store') {
...
// 🔹 slug من query ?slug=
      ...
      // ✅ إذا ما عندنا ID صالح → افتح بالـ slug فقط
    ...
    GetPage(name: orderDetails, page: () {
      return getRoute(Get.arguments ?? OrderDetailsScreen(
        ...
        orderModel: null, fromNotification: Get.parameters['from'] == 'true',
    ...
    GetPage(name: payment, page: () {
      OrderModel order = OrderModel(
        ...
        , orderType: Get.parameters['type'], userId: int.parse(Get.parameters['user']!),
    ...
    GetPage(name: addAddress, page: () => getRoute(AddAddressScreen(
      fromCheckout: Get.parameters['page'] == 'checkout', fromRide: Get.parameters['ride'] == 'true',zoneId: int.tryParse(Get.parameters['zone_id'] ?? ''),
    ...
    GetPage(name: itemDetails, page: () => getRoute(Get.arguments ?? ItemDetailsScreen(itemId:int.tryParse(Get.parameters['id'] ?? '') ?? 0
    ...
    GetPage(name: brandsItemScreen, page: () => BrandsItemScreen(
      brandId: int.tryParse(Get.parameters['brandId'] ?? '') ?? 0,
  ...
  static Widget getRoute(
    ...
    // ⛔ Cold Start: لم يكتمل تحميل الكونفيق بعد
    ...
    if (GetPlatform.isAndroid) {
      minimumVersion = splash.configModel!.appMinimumVersionAndroid ?? 0;
    } else if (GetPlatform.isIOS) {
      minimumVersion = splash.configModel!.appMinimumVersionIos ?? 0;
    }

    return (AppConstants.appVersion < minimumVersion && !GetPlatform.isWeb)
        ? const UpdateScreen(isUpdate: true)
        : splash.configModel!.maintenanceMode == true
