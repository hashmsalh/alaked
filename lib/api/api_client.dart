...
import 'dart:async';
import 'package:sixam_mart/helper/image_compress_helper.dart';
class ApiClient extends GetxService {
  final String appBaseUrl;
  final SharedPreferences sharedPreferences;
  static final String noInternetMessage = 'connection_to_api_server_failed'.tr;
  final int timeoutInSeconds =150;
  ...
  Future<Response> getData(String uri, {Map<String, dynamic>? query, Map<String, String>? headers, bool handleError = true}) async {
    try {
      ...
      if (e is TimeoutException) {
        return Response(
          statusCode: 408,
          statusText: 'الإنترنت بطيء أو حجم الصورة كبير، يرجى المحاولة مرة أخرى',
        );
      }

      return Response(statusCode: 1, statusText: 'حدث خطأ في الاتصال بالخادم');
    }
  }
      ...
      for(MultipartBody multipart in multipartBody) {
        if(multipart.file != null) {
          if(kIsWeb) {
            ...
            File originalFile = File(multipart.file!.path);
            ...
            if (fileSizeInMB > 2) {
              // نضغط فقط إذا أكبر من 2 ميجا
              File? compressedFile =
              await ImageCompressHelper.compressImage(originalFile);
